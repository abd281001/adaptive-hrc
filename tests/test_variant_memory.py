...
class VariantMemoryTests(unittest.TestCase):
    ...
    def test_register_creates_variant(self):
        ...
        self.assertEqual(self.memory.latest[TOMATO_ONION_SOUP], v.pref_hash)
    ...
    def test_coexistence_two_variants(self):
        ...
        self.assertNotEqual(v1.pref_hash, v2.pref_hash)
        self.assertEqual(len(self.memory.variants[TOMATO_ONION_SOUP]), 2)
        self.assertEqual(self.memory.latest[TOMATO_ONION_SOUP], v2.pref_hash)

    def test_promote_changes_latest(self):
        v1 = self.memory.register(TOMATO_ONION_SOUP, ["a", "b"], step=1)
        self.memory.register(TOMATO_ONION_SOUP, ["b", "a"], step=2)
        self.memory.promote_latest(TOMATO_ONION_SOUP, v1.pref_hash, step=3)
        self.assertEqual(self.memory.latest[TOMATO_ONION_SOUP], v1.pref_hash)

    def test_library_filters_allowed_keys(self):
        v1 = self.memory.register(TOMATO_ONION_SOUP, ["a", "b"], step=1)
        self.memory.register(TOMATO_ONION_SOUP, ["b", "a"], step=2)
        library = self.memory.library(allowed_keys={(TOMATO_ONION_SOUP, v1.pref_hash)})
        self.assertEqual(len(library), 1)
        self.assertEqual(library[0].pref_hash, v1.pref_hash)
