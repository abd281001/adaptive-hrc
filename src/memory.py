...
class Classification:
    ...
    pref_hash: Optional[str]   # matched variant hash when kind=="known"; field name kept for compatibility
    jaccard: float
...
def _jaccard_with_cached_b(a: Sequence[str], cb_full: Counter, cb_type: Counter) -> float:
    """Same as `jaccard` but reuses pre-computed counters for `b` (the variant)."""
    ca_full = Counter(a)
    ca_type = Counter(_action_type(x) for x in a)
    return 0.70 * _jaccard_counters(ca_full, cb_full) + 0.30 * _jaccard_counters(ca_type, cb_type)
...
def variant_hash(sequence: Sequence[str], canonicalize: bool = False) -> str:
    """Stable order-preserving hash for a demonstrated recipe-preference variant. `canonicalize` is accepted for backwards-compatible call sites, but no longer collapses same-type swaps. In this domain, ingredient order, cleanup order, and equipment order are potentially meaningful user preferences."""
    parts = [f"{i}:{a}" for i, a in enumerate(sequence)]
    payload = "\x1f".join(parts).encode("utf-8")
    return hashlib.sha256(payload).hexdigest()[:16]

def pref_hash(sequence: Sequence[str], canonicalize: bool = False) -> str:
    """Deprecated alias for `variant_hash`; kept for older notebooks/tests."""
    return variant_hash(sequence, canonicalize=canonicalize)
...
class Disambiguator:
    ...
    def classify(self, sequence: Sequence[str], library: List[KnownVariant], threshold: Optional[float] = None) -> Classification:
        ...
        with self._profile("classify"):
            ...
            # Score every known variant. Variant-side counters are cached on the KnownVariant counters are cached lazily on first access.
            scored: List[Tuple[KnownVariant, float, float]] = []
            for v in library:
                j = _jaccard_with_cached_b(sequence, v.counter_full(), v.counter_type())
                o = kendall_tau_distance(sequence, v.ordering, unmatched_penalty=getattr(self.cfg, "ordering_unmatched_penalty", 0.5))
                scored.append((v, j, o))
            ...
            # Same recipe. Does any variant have the same ordering?
            for v, j, o in scored:
                if (v.recipe_id == best_recipe_id and j >= thr and o <= self.cfg.ordering_tolerance and len(v.ordering) == len(sequence) and all(x == y for x, y in zip(v.ordering, sequence))): return Classification("known", v.recipe_id, v.pref_hash, j, o)
...
class VariantMemory:
    ...
    def __init__(self, cfg: Config = DEFAULT_CONFIG):
        self.cfg = cfg
        self.variants: Dict[str, Dict[str, Variant]] = defaultdict(dict)    # recipe_id -> insertion-ordered dict of pref_hash -> Variant
        self.latest: Dict[str, str] = {}                                    # recipe_id -> pref_hash of most-recently-observed variant.
        self.last_evicted_key: Optional[VariantKey] = None                  # Populated by register() when max_variants_per_recipe evicts an older variant. The agent consumes this to keep DecayManager in sync.
    ...
    def register(self, recipe_id: str, ordering: Sequence[str], step: int) -> Variant:
        ...
        if h in slot:
            ...
            v = Variant(recipe_id=recipe_id, pref_hash=h, ordering=tuple(ordering), first_seen_step=step, last_seen_step=step)
            slot[h] = v
            if len(slot) > self.cfg.max_variants_per_recipe:
                evict = min(slot.values(), key=lambda x: x.last_seen_step).pref_hash
                if evict != h:
                    del slot[evict]
                    self.last_evicted_key = (recipe_id, evict)
        self.latest[recipe_id] = h
        return v

    def promote_latest(self, recipe_id: str, pref_hash_: str, step: int) -> None:
        if pref_hash_ in self.variants.get(recipe_id, {}):
            self.latest[recipe_id] = pref_hash_
            self.variants[recipe_id][pref_hash_].last_seen_step = step
    ...
    def variants_of(self, recipe_id: str, allowed_keys: Optional[Set[VariantKey]] = None) -> List[KnownVariant]:
        return [KnownVariant(v.recipe_id, v.pref_hash, v.ordering) for v in self.variants.get(recipe_id, {}).values() if allowed_keys is None or (v.recipe_id, v.pref_hash) in allowed_keys]
    ...
    def latest_variant(self, recipe_id: str, allowed_keys: Optional[Set[VariantKey]] = None) -> Optional[Variant]:
        ...
        candidates = [v for v in slot.values() if (v.recipe_id, v.pref_hash) in allowed_keys]
...
class Entry:
    ...
    @property
    def key(self) -> VariantKey:    return (self.recipe_id, self.pref_hash)
...
class DecayManager:
    ...
    def __init__(self, cfg: Config = DEFAULT_CONFIG):
        ...
        self._active_count_window: Deque[int] = deque(maxlen=cfg.mwr_window)
        self._active_recipe_count_window = self._active_count_window
        ...
        # Legacy name retained for metrics compatibility. Entries are all same-variant reuses: (step, key, reuse_gap).
        self.reentry_events: List[Tuple[int, VariantKey, int]] = []
        self._last_count_append_step: int = -1   # guards _active_count_window deduplication
        self._recompute_effective(step=0)
        
    # Backwards-compat alias for callers that read the old single window.
    @property
    def _gap_window(self) -> Deque[int]: return self._reuse_gap_window

    # core ops
    def register(self, recipe_id: str, pref_hash: str, ordering: Tuple[str, ...], now: int, cycle: int) -> Entry:
        """Insert, reset an active variant, or restore a pruned variant.  Weight is 1.0 afterwards in all three cases. """
        key = (recipe_id, pref_hash)
        if key in self.active:
            ...
            e.ordering = ordering
            if self.cfg.protect_latest_preference: self.mark_latest(recipe_id, pref_hash)
        ...
        e = Entry(recipe_id=recipe_id, pref_hash=pref_hash, ordering=ordering, weight=1.0, added_step=now, added_cycle=cycle, last_seen_step=now, prior_lifespan=prior_lifespan)
        self.active[key] = e
        if self.cfg.protect_latest_preference: self.mark_latest(recipe_id, pref_hash)
        self._recompute_effective(step=now)
        self._log_weight(now, e)
    ...
    def mark_latest(self, recipe_id: str, pref_hash: str) -> None:
        ...
        self.latest_by_recipe[recipe_id] = pref_hash
        key = (recipe_id, pref_hash)
        self.latest_keys.add(key)
        if key in self.active: self.active[key].weight = 1.0

    def unmark_latest(self, recipe_id: str, pref_hash: str) -> None:
        key = (recipe_id, pref_hash)
        if self.latest_by_recipe.get(recipe_id) == pref_hash: del self.latest_by_recipe[recipe_id]
        self.latest_keys.discard(key)

    def discard(self, recipe_id: str, pref_hash: str) -> None:
    ...
    def active_count_window_snapshot(self) -> List[int]:
        """Backwards-compatible alias for active_recipe_count_window_snapshot."""
        return self.active_recipe_count_window_snapshot()

    # rate adaptation
    def _record_reuse_gap(self, key: VariantKey, gap: int, step: Optional[int] = None) -> None:
        """Record any same-recipe_pref reuse gap for diagnostics."""
        if gap <= 0: return
        gap_i = int(gap)
    ...
    def _recompute_effective(self, step: int) -> None:
        ...
        # Append at most once per step to keep _active_count_window at session-level granularity (one entry per completed session, not one entry per internal recompute call within a session).
    ...
    def _prune_entry(self, key: VariantKey, entry: Entry, now: int, cycle: int) -> None:
        self.latest_keys.discard(key)
        if self.latest_by_recipe.get(entry.recipe_id) == entry.pref_hash: del self.latest_by_recipe[entry.recipe_id]
        self.pruned[key] = PrunedEntry(recipe_id=entry.recipe_id, pref_hash=entry.pref_hash, ordering=entry.ordering, added_step=entry.added_step, removed_step=now, added_cycle=entry.added_cycle, removed_cycle=cycle, last_seen_step=entry.last_seen_step)
        del self.active[key]
...
# Backwards-compatible wrappers so existing tests/baselines keep working. Reuse gaps are recorded, but base_rate stays fixed under the original active-variant decay rule.
def _adapt_base_from_reuse(self: "DecayManager", gap: int, step: Optional[int] = None) -> None:
    if gap <= 0: return
    self._reuse_gap_window.append(int(gap))
    self._refresh_base_from_reuse_window()

DecayManager._adapt_base_from_reuse = _adapt_base_from_reuse  # type: ignore
DecayManager._adapt_rate_from_reuse = _adapt_base_from_reuse  # type: ignore
DecayManager._rescale_rate_by_size  = DecayManager._recompute_effective  # type: ignore
