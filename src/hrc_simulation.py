...
def _predict(
    ...
    return Prediction(
        ...
        correct_top_1=bool(ranked and ranked[0] == actual),
        correct_top_k=actual in ranked,
        top_1_tie_size=top_probability_tie_size(distribution),
        prediction_time=float(elapsed),
    )
...
def simulate_episode(
    *,
    ...
    predict_distribution: Callable[[Sequence[str]], Mapping[str, float]],
    observe_ground_truth: Callable[
        [Any, Optional[Mapping[str, float]], Optional[str]], None
    ],
    top_k: int,
    min_probability: float,
    tie_rng: Optional[np.random.Generator] = None,
    timing: Timing = DEFAULT_TIMING,
    capture_robot_metadata: Optional[Callable[[RobotDecision], Mapping[str, Any]]] = None,
    on_robot_feedback: Optional[Callable[[RobotDecision], None]] = None,
) -> EpisodeTrace:
    ...
    for position, action in enumerate(actual_actions):
        reference_positions[action].append(position)
    ...
    for index, (observation, actual) in enumerate(zip(observations, actual_actions)):
        ...
        if not robot_turn_next:
            human_turn_count += 1
            # Shadow predictions never control execution. Together with the
            # robot-turn predictions they score every ground-truth prefix.
            prediction = _predict(
                index, prefix, actual, predict_distribution, top_k, decision_rng,
            )
            teacher_forced_top_1_tie_count += int(
                prediction.top_1_tie_size > 1
            )
            human_turns.append(HumanTurn(
                **vars(prediction),
                human_turn_index=human_turn_count - 1,
            ))
            human_action_time += timing.human_action_time
            total_time += timing.human_action_time
            observe_ground_truth(
                observation, prediction.distribution, prediction.predicted,
            )
        ...
        next_position = (
            ...
            if next_position_index < len(positions) else None
        )
        matching_reference_offset = (
            next_position - index if next_position is not None else None
        )
        matches_later_reference_action = bool(
            not prediction.correct_top_1
            and matching_reference_offset is not None
        )
        decision = RobotDecision(
            **vars(prediction),
            robot_turn_index=robot_turn_index,
            matches_later_action=(
                matches_later_reference_action
            ),
            matching_action_offset=matching_reference_offset,
        )
        ...
        log_loss -= math.log(
            max(float(decision.distribution.get(actual, floor)), floor)
        )
        ...
        metadata = (
            dict(capture_robot_metadata(decision) or {})
            if capture_robot_metadata is not None else {}
        )
        if on_robot_feedback is not None:
            on_robot_feedback(decision)
        total_time += step_time
        human_actions = human_turn_count + human_correction_count
        human_effort = (
            human_turn_count * timing.human_action_time
            + human_correction_count * timing.human_correction_time
        )
        robot_turns.append(RobotTurn(
            **vars(decision),
            scheduled_actor="robot",
            executed_by=executed_by,
            human_corrected=not decision.correct_top_1,
            step_time=float(step_time),
            elapsed_time=float(total_time),
            human_actions=human_actions,
            human_time=float(human_effort),
            metadata=metadata,
        ))
        robot_turn_index += 1
        observe_ground_truth(
            observation, decision.distribution, decision.predicted,
        )
    ...
    return EpisodeTrace(
        ...
        human_shadow_turns=tuple(human_turns),
        summary=EpisodeSummary(
            recipe_steps=len(actual_actions),
            robot_turn_count=robot_turn_index,
            ...
            later_action_errors=later_action_errors,
            top_1_hits=top_1_hits,
            top_k_hits=top_k_hits,
            robot_top_1_tie_count=robot_top_1_tie_count,
            teacher_forced_top_1_tie_count=teacher_forced_top_1_tie_count,
            ...
            correct_robot_time=float(correct_robot_time),
            wrong_robot_time=float(wrong_robot_time),
            human_action_time=float(human_action_time),
