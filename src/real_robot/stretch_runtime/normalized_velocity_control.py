...
class NormalizedVelocityControl():
    def __init__(self, robot, command_watchdog_s=0.30):
        self.robot = robot
    ...
    def _init_command(self):
        with self.lock:
            ...
            self.command = {'num': 0, 'time': time.monotonic(), 'cmd': None}
    ...
    def _safety_stop(self):
        ...
        self.gripper.command_stick_to_motion(0, self.robot)
    ...
    def _execute(self, norm_vel_cmd):
        ...
        if cmd is not None: 
            ...
            self._last_executed_nonzero = any(abs(float(value)) > 1e-9 for value in cmd.values())
