...
class ArucoMarker:
    def __init__(self, aruco_id, marker_info, show_debug_images=False):
        ...
        self.quality_reason = "not_updated"
    ...
    def update(self, corners, frame_number, rgb_camera_info):
        ...
        solved, rvecs_ret, tvecs_ret = cv2.solvePnP(objectPoints=points_3D,
                                                     imagePoints=self.corners,
                                                     cameraMatrix=self.camera_matrix,
                                                     distCoeffs=self.distortion_coefficients)
        if not solved:
            self.quality_reason = "solve_pnp_failed"
            return
        ...
        self.min_dist_between_corners = minimum_distance_between_corners(self.corners)
        projected, _ = cv2.projectPoints(
            points_3D, self.aruco_rotation, tvecs_ret,
            self.camera_matrix, self.distortion_coefficients,
        )
        projected = projected.reshape(-1, 2)
        self.reprojection_error_px = float(
            np.sqrt(np.mean(np.sum((projected - self.corners) ** 2, axis=1)))
        )
        min_pixels = float(self.info.get("min_marker_pixels", 0.0))
        max_error = float(self.info.get("max_reprojection_error_px", float("inf")))
        min_depth = float(self.info.get("min_marker_depth_m", 0.0))
        max_depth = float(self.info.get("max_marker_depth_m", float("inf")))
        if not np.all(np.isfinite(self.aruco_position)):
            self.quality_reason = "non_finite_pose"
            return
        if self.min_dist_between_corners < min_pixels:
            self.quality_reason = "marker_too_small"
            return
        if self.reprojection_error_px > max_error:
            self.quality_reason = "reprojection_error"
            return
        if not min_depth <= float(aruco_depth_estimate) <= max_depth:
            self.quality_reason = "implausible_depth"
            return
        R = np.identity(4)
...
class ArucoMarkerCollection:
    ...
    def __iter__(self):
        ...
        for k in keys:
            ...
            if marker.frame_number == self.frame_number and marker.ready:
                yield marker
    ...
    def update(self, rgb_image, rgb_camera_info):
        ...
        ids = [] if self.aruco_ids is None else [int(value) for value in self.aruco_ids]
        self.duplicate_ids = {value for value in ids if ids.count(value) > 1}
        ...
        if self.aruco_ids is not None: 
            for corners, aruco_id in zip(self.aruco_corners, self.aruco_ids):
                ...
                if aruco_id in self.duplicate_ids:
                    self.collection[aruco_id].ready = False
                    self.collection[aruco_id].quality_reason = "duplicate_id_in_frame"
                else:
    ...
    def get_detected_marker_dict(self):
        ...
        for m in self.aruco_marker_collection:
            ...
            out[aruco_id] = {'pos': pos,
                             ...
                             'corners': np.array(m.corners),
                             'center': np.mean(m.corners, axis=0),
                             'min_dist_between_corners': min_dist_between_corners,
                             'reprojection_error_px': m.reprojection_error_px,
