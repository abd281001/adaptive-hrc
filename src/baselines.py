...
class BehaviorCloner:
    ...
    def _prepare_examples(self, demonstrations: Sequence[Demonstration], demo_weights: Sequence[float]) -> Tuple[np.ndarray, np.ndarray, np.ndarray, Dict[str, int], Dict[int, str]]:
        ...
        state_matrix, _, _ = build_features(state_vectors, normalizer=self.normalizer, update_normalizer=True)
        ...
        for demo, demo_weight in zip(demonstrations, demo_weights):
            ...
            for state, action in demo:
                ...
                feature_rows.append(
                    self._compose_features(state_matrix[state_id], prefix)
                )
                targets.append(action_ids[action])
                example_weights.append(weight)
                prefix.append(action)
        if not feature_rows:
            return (np.zeros((0, self.state_dim + self._history_dim()), dtype=np.float32),  np.zeros(0, dtype=np.int64),    np.zeros(0, dtype=np.float32),      action_ids,  action_labels)
        return (
            np.vstack(feature_rows).astype(np.float32),
            np.asarray(targets, dtype=np.int64),
            np.asarray(example_weights, dtype=np.float32),
            action_ids,
            action_labels,
        )
    ...
    def _warm_start(
        ...
        previous_bias: Optional[np.ndarray],
        previous_action_ids: Dict[str, int],
        previous_state_dim: int,
        previous_history_ids: Dict[str, int],
        previous_history_size: int,
        ...
        for action, current_index in action_ids.items():
            ...
            if state_rows > 0:
                weights[:state_rows, current_index] = previous_weights[:state_rows, previous_index]
            for lag_index in range(shared_history_size):
                ...
                for action, current_action_index in self.history_action_ids.items():
                    ...
                    if previous_row < previous_weights.shape[0] and current_row < weights.shape[0]:
                        weights[current_row, current_index] = previous_weights[previous_row, previous_index]
            if previous_length_row < previous_weights.shape[0] and length_row < weights.shape[0]:
                weights[length_row, current_index] = previous_weights[previous_length_row, previous_index]
    ...
    def fit(self, demonstrations: Sequence[Demonstration], demo_weights: Optional[Sequence[float]] = None) -> None:
        ...
        feature_rows, targets, sample_weights, action_ids, action_labels = (
            self._prepare_examples(demonstrations, weights)
        )
        ...
        if warm_start:
            model_weights, model_bias = self._warm_start(
                feature_dim,
                action_ids,
                previous_weights,
                previous_bias,
        ...
        for _ in range(max(1, epochs)):
            ...
            for start in range(0, len(indices), batch_size):
                ...
                weight_gradient = (
                    batch_features.T @ delta
                ).astype(np.float32) + l2_penalty * model_weights
    ...
    def predict(self, state: StateVector, prefix: Sequence[str]) -> Dict[str, float]:
        ...
        features, _, _ = build_features({0: tuple(state)}, normalizer=self.normalizer)
...
class UnpinnedAgent(BaselineAgent):
    ...
    def __init__(self, settings: Settings = DEFAULT_SETTINGS, **kwargs):
        super().__init__(
            settings=_without_proposed_components(settings), **kwargs,
        )
...
class FrozenAgent(BaselineAgent):
    ...
    def lock_deployment(self, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        self._offline_pretraining_metadata = {
            ...
            "comparison_scope": "training_regime_reference_not_method_replication",
            "updates_allowed": False,
            "train_count": int(len(total_times)),
            "train_wall_s": float(sum(total_times)),
            "fit_wall_s": float(sum(fit_times)),
            "build_wall_s": float(sum(build_times)),
            "train_flops": float(sum(flops)),
            "train_variants": int(len(self.replay.active)),
            "train_steps": int(
                sum(len(entry.ordering) for entry in self.replay.active.values())
            ),
            **dict(metadata or {}),
        }
        self._clear_episode()
    ...
    def policy_stats(self) -> Dict[str, Any]:
        if self._deployment_locked:
            return {
                **dict(self._deployment_action_policy),
        ...
        *,
        ...
        if not self._deployment_locked:
            return super().observe(
                ...
                precomputed_prediction=precomputed_prediction,
            )
        self._deployment_step += 1
        first_online_action = not self.current_prefix
        distribution = (
            {} if first_online_action else
            (dict(precomputed_distribution) if precomputed_distribution is not None else self.predict_actions(self.current_prefix))
        )
        predicted = (
...
class BehaviorCloningAgent(BaselineAgent):
    ...
    def __init__(self, settings: Settings = DEFAULT_SETTINGS, **kwargs):
        ...
        self.replay = ReplayMemory(self.settings, policy="none")
        self.cloner = BehaviorCloner(settings=self.settings)
    ...
    def _fit_predictors(
        self, trajectories, weights, *, warm_start: bool, records=None,
    ) -> None:
    ...
    def audit_pruning(self, max_prefixes: int = 24, tolerance: float = 5e-2) -> Dict[str, object]:
        ...
        fresh_cloner = BehaviorCloner(settings=self.settings)
        fresh_cloner.fit(trajectories, weights)
        return self._compare_policies(
            entries,
            lambda state, prefix: self.cloner.predict(state, prefix),
            lambda state, prefix: fresh_cloner.predict(state, prefix),
...
class EwcAgent(BaselineAgent):
    ...
    @staticmethod
    def _resize_to(
        vector: Optional[np.ndarray], size: int,
    ) -> np.ndarray:
    ...
    def _record_demo(
        ...
        *,
        ...
        if entry is not None:
            self._pending_demos.append(entry)
        else:
            self._pending_demos.append({"ordering": tuple(ordering), "transitions": tuple(transitions)})

    @staticmethod
    def _estimate_fisher_flops(
        trajectories: Sequence[Demonstration], feature_count: int,
    ) -> float:
        state_visits = sum(len(demo) for demo in trajectories)
        return float(
            3.0 * max(0, state_visits) * max(1, int(feature_count))
        )
    ...
    def _record_costs(self, auxiliary_stats: Dict[str, Any], fisher_flops: float) -> None:
        ...
        try:
            primary_flops = float(primary_estimate)
        except (TypeError, ValueError):
            primary_flops = 0.0
        try:
            auxiliary_flops = float(auxiliary_estimate)
        except (TypeError, ValueError):
            auxiliary_flops = 0.0
        if not math.isfinite(primary_flops):
            primary_flops = 0.0
        if not math.isfinite(auxiliary_flops):
            auxiliary_flops = 0.0
        try:
            measured_fisher_flops = float(fisher_flops)
        except (TypeError, ValueError):
            measured_fisher_flops = 0.0
        if not math.isfinite(measured_fisher_flops):
            measured_fisher_flops = 0.0
        self._custom_fit_stats = {
            "estimated_flops": float(
                primary_flops + auxiliary_flops + measured_fisher_flops
            ),
            "ewc_primary_flops": float(primary_flops),
            "ewc_auxiliary_flops": float(auxiliary_flops),
            "ewc_fisher_flops": float(measured_fisher_flops),
            "ewc_aux_iterations_run": float(auxiliary_stats.get("iterations_run", 0.0) or 0.0),
            "ewc_aux_n_states": float(auxiliary_stats.get("n_states", 0.0) or 0.0),
            "ewc_aux_n_features": float(auxiliary_stats.get("n_features", 0.0) or 0.0),
        }
    ...
    def _fit_predictors(
        ...
        self.maxent.fit(
            ...
            ewc_anchor=self._anchor,
            ewc_fisher=self._fisher,
        )
        ...
        task_maxent = type(self.maxent)(settings=self.settings)
        task_maxent.fit(
            task_trajectories,
            task_weights,
            ewc_anchor=self._anchor,
        ...
        self._fisher_total = self._resize_to(
            self._fisher_total, feature_count,
        )
        self._weighted_anchor = self._resize_to(
            self._weighted_anchor, feature_count,
        )
        ...
        self._anchor = (
            self._weighted_anchor / safe_fisher
        ).astype(np.float32)
        self._fisher = np.clip(
            self._fisher_total, 0.0, fisher_cap
        ).astype(np.float32)
...
class ReplayBcAgent(BehaviorCloningAgent):
    ...
    def _add_to_buffer(self, record: Any) -> None:
        ...
        self._seen += 1
        stored = self._buffer_item(
            self._store_demo(record),
            getattr(record, "transitions", ()),
    ...
    def _retrain(self) -> None:
        ...
        eligible = [
            record for record in self._buffer
            if record.key is None or record.key in active
        ]
        if not eligible:
        ...
        base_weights = [
            float(self.replay.active[record.key].weight)
            if record.key in self.replay.active else 1.0
            for record in demos
        ]
        ...
        self._fit_predictors(
            trajectories, weights, warm_start=False, records=demos,
        )
