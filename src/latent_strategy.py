...
ROLES: Tuple[str, ...] = ("ingredient_retrieval", "equipment_setup", "prep", "load", "activate", "process", "shutdown", "serving_setup", "serve", "cleanup_move", "wash", "other")
_ROLE_INDEX = {role: index for index, role in enumerate(ROLES)}
_PAIR_OFFSET = 2 * len(ROLES)
_PAIR_INDEX = {(first, second): _PAIR_OFFSET + index for index, (first, second) in enumerate((a, b) for a in range(len(ROLES)) for b in range(a + 1, len(ROLES)))}
FINGERPRINT_DIM = _PAIR_OFFSET + len(_PAIR_INDEX)


def _role_schema(roles: Sequence[str]) -> Tuple[Dict[str, int], int, Dict[Tuple[int, int], int], int]:
    names = tuple(map(str, roles))
    if not names or len(set(names)) != len(names):
        raise ValueError("latent strategy roles must be non-empty and unique")
    role_index = {role: index for index, role in enumerate(names)}
    pair_offset = 2 * len(names)
    pair_index = {(first, second): pair_offset + index for index, (first, second) in enumerate((a, b) for a in range(len(names)) for b in range(a + 1, len(names)))}
...
def action_role(label: str) -> str:
    ...
    verb = action.verb
    item = (action.get("item") or action.get("container") or action.get("vessel") or "")
    source = action.get("from")
    destination = action.get("to")
    if verb == "transfer" and source == "storage" and item in INGREDIENTS:      return "ingredient_retrieval"
    if (verb in {"transfer", "move_container"} and source == "storage" and item in CONTAINERS and item not in {"plate", "glass"}): return "equipment_setup"
    if verb in {"cut", "grate"}:                                                return "prep"
    if verb in {"load", "unload"}:                                              return "load"
    if verb == "turn_on":                                                       return "activate"
    if verb in {"cook", "cook_contents", "blend", "combine"}:                   return "process"
    if verb == "turn_off":                                                      return "shutdown"
    if (verb == "transfer" and item in {"plate", "glass"} and destination in {"plating_station", "serving_station"}):           return "serving_setup"
    if (verb == "serve" or (verb == "move_container" and item in {"plate", "glass"} and destination == "serving_station")):     return "serve"
    if verb in {"transfer", "move_container"} and destination == "washing_station":                                             return "cleanup_move"
    if verb == "wash":                                                                                                          return "wash"
    return "other"


def _role_positions(actions: Sequence[str], role_fn: Callable[[str], str] = action_role, *, roles: Sequence[str] = ROLES) -> Tuple[Tuple[str, ...], Dict[int, list[int]]]:
    role_names      = tuple(map(str, roles))
    role_index, _pair_offset, _pair_index, _width = _role_schema(role_names)
    observed_roles  = tuple(role_fn(action) for action in actions if action != "stop")
    positions: Dict[int, list[int]] = {}
    for position, role in enumerate(observed_roles):
        if role not in role_index: raise ValueError(f"domain action role {role!r} is absent from {role_names!r}")
        positions.setdefault(role_index[role], []).append(position)
    return observed_roles, positions


def workflow_fingerprint(actions: Sequence[str], role_fn: Callable[[str], str] = action_role, *, roles: Sequence[str] = ROLES) -> np.ndarray:
    """Encode role timing/counts and pairwise precedence without identities."""
    role_names = tuple(map(str, roles))
    _indices, _pair_offset, pair_index, width = _role_schema(role_names)
    observed_roles, positions = _role_positions(actions, role_fn, roles=role_names)
    denominator = max(1, len(observed_roles) - 1)
    ...
    for role_index, indices in positions.items():
        ...
        out[len(role_names) + role_index] = min(1.0, len(indices) / 3.0)
    for (first, second), feature_index in pair_index.items():
...
class LatentStrategyResidual:
    ...
    def __init__(self, settings: Any, domain: Optional[Any] = None):
        self.settings = settings
        if domain is None:
            from .domain import default_domain
            domain = default_domain()
        self.domain = domain
        self.roles = tuple(getattr(self.domain, "strategy_roles", ROLES))
        (self.role_index, self.pair_offset, self.pair_index, self.fingerprint_dim) = _role_schema(self.roles)
        self.reset()

    def reset(self) -> None:
        self.mean               = np.zeros(self.fingerprint_dim, dtype=np.float32)
        self.scale              = np.ones(self.fingerprint_dim, dtype=np.float32)
        self.components         = np.zeros((0, self.fingerprint_dim), dtype=np.float32)
        self.codes              = np.zeros((0, 0), dtype=np.float32)
        self.code_scale         = np.ones(0, dtype=np.float32)
        self.fingerprints       = np.zeros((0, self.fingerprint_dim), dtype=np.float32)
        self.role_counts        = np.zeros((0, len(self.roles)), dtype=np.int16)
    ...
    def fit(self, demonstrations: Sequence[Sequence[Tuple[Tuple[int, ...], str]]], demo_weights: Sequence[float] | None = None) -> None:
        ...
        if not action_rows: return
        weights         = np.asarray([float(demo_weights[index]) for index, _row in indexed_rows] if demo_weights is not None and len(demo_weights) == len(demonstrations) else [1.0] * len(action_rows), dtype=np.float64)
        if weights.shape != (len(action_rows),) or float(weights.sum()) <= 0.0: weights = np.ones(len(action_rows), dtype=np.float64)
        weights         = np.maximum(weights, 0.0)
        normalized      = weights / max(float(weights.sum()), 1e-12)
        matrix          = np.stack([workflow_fingerprint(row, self.domain.action_role, roles=self.roles) for row in action_rows])
        ...
        code_variance   = np.average(self.codes ** 2, axis=0, weights=normalized)
        ...
        self.last_fit_stats = {
            ...
            "latent_strategy_fingerprint_dim": self.fingerprint_dim,
            "latent_strategy_rank": rank,
            "latent_strategy_prototypes": len(action_rows),
            "latent_strategy_parameter_count": int(self.components.size + self.mean.size + self.scale.size),
            "latent_strategy_prototype_storage_values": int(self.fingerprints.size + self.codes.size + self.role_counts.size + sum(len(sequence) for sequence in self.role_sequences)),
            "latent_strategy_fit_flops": flop_estimate, "latent_strategy_flop_scope": "weighted_pca_estimate_only"}

    def _partial_observation(self, prefix: Sequence[str]) -> Tuple[np.ndarray | None, Tuple[int, ...], int, int]:
        roles, positions = _role_positions(prefix, self.domain.action_role, roles=self.roles)
        observed = sorted(positions)
        if not observed or not len(self.components): return None, (), len(observed), 0
        query = np.zeros(self.fingerprint_dim, dtype=np.float32)
        mask: list[int] = []
        denominator = max(1.0, self.mean_demo_length - 1.0)
        means = {index: float(np.mean(values)) for index, values in positions.items()}
        for role_index in observed: query[role_index] = means[role_index] / denominator
        relations = 0
        for (first, second), feature_index in self.pair_index.items():
            if first in means and second in means:
                query[feature_index] = (1.0 if means[first] < means[second] else -1.0)
    ...
    def score(self, prefix: Sequence[str], candidates: Sequence[str], *, decision_prefix: Sequence[str] | None = None) -> StrategyScore:
        ...
        logits = -selected_distances / bandwidth + np.log(np.maximum(self.weights[indices], 1e-8))
        ...
        code_center = np.average(normalized_codes, axis=0, weights=neighbor_weights)
        code_dispersion = float(np.average(np.mean((normalized_codes - code_center) ** 2, axis=1), weights=neighbor_weights))
        latent_agreement = 1.0 / (1.0 + code_dispersion)

        roles_by_action = {str(action): self.role_index[self.domain.action_role(str(action))] for action in candidates}
        prefix_roles, prefix_positions = _role_positions(prefix if decision_prefix is None else decision_prefix, self.domain.action_role, roles=self.roles)
        ...
        for action, role_index in roles_by_action.items():
            ...
            for neighbor_index, fingerprint in zip(indices, self.fingerprints[indices]):
                ...
                per_neighbor.append(timing if observed_count < expected_count else -1.0)
        ...
        if sequence_weight > 0.0 and prefix_roles:
            role_mass = np.zeros(len(self.roles), dtype=np.float64)
            confidence_mass = 0.0
            for neighbor_index, neighbor_weight in zip(indices, neighbor_weights):
                next_role, alignment, flops = self._aligned_next_role(tuple(self.role_index[role] for role in prefix_roles), self.role_sequences[neighbor_index])
    ...
    def _aligned_next_role(observed: Tuple[int, ...], prototype: Tuple[int, ...],) -> Tuple[int | None, float, float]:
        ...
        if not prototype: return None, 0.0, 0.0
        gap = 0.35
        previous = np.arange(len(prototype) + 1, dtype=np.float64) * gap
        for observed_role in observed:
            current = np.empty_like(previous)
            current[0] = previous[0] + gap
            for column, prototype_role in enumerate(prototype, start=1): current[column] = min(previous[column] + gap, current[column - 1] + gap, previous[column - 1] + (observed_role != prototype_role))
            previous = current
    ...
    def supports_correction(self, actual: str, predicted: str) -> bool:
        ...
        return bool(utilities and str(actual) in utilities and str(predicted) in utilities and float(utilities[str(actual)]) > float(utilities[str(predicted)]) and self.last_score.confidence > 0.0 and self.last_score.coverage > 0.0)


def fuse_strategy_residual(base: Mapping[str, float], score: StrategyScore, strength: float) -> Tuple[Dict[str, float], float]:
    """Apply a bounded log-linear residual while preserving full support."""
    if not base or not score.utilities: return dict(base), 0.0
    ...
    alpha = (max(0.0, float(strength)) * score.confidence * score.coverage * max(0.0, 1.0 - margin))
    if alpha <= 1e-8: return dict(base), 0.0
    adjusted = {action: max(float(probability), 1e-12) * math.exp(alpha * float(score.utilities.get(action, 0.0))) for action, probability in base.items()}
    total = sum(adjusted.values())
    if total <= 0.0 or not math.isfinite(total): return dict(base), 0.0
